...
def main():
    ...
    parser.add_argument('--action-repeat', type=int,
                        default=4)  # cheetah 4; cartpole 8; reacher 4; finger 2; cup 4; walker 2
    parser.add_argument('--state-dim', type=int,
                        default=30)  # Distributions in latent space are 30-dimensional diagonal Gaussians with predicted mean and standard deviation.
    parser.add_argument('--rnn-hidden-dim', type=int,
                        default=200)  # a GRU (Cho et al., 2014) with 200 units as deterministic path in the dynamics model
    ...
    parser.add_argument('--epsilon', type=float,
                        default=1e-4)  # The model is trained using the Adam optimizer (Kingma & Ba, 2014) with a learning rate of 1e-3, epsilon = 1e−4
    parser.add_argument('--clip-grad-norm', type=int, default=1000)  # gradient clipping norm of 1000
    parser.add_argument('--free-nats', type=int,
                        default=3)  # grant the model 3 free nats by clipping the divergence loss below this value
    """
    For planning, we use:
    ...
    for episode in range(args.seed_episodes, args.all_episodes):
        ...
        # 收集经验
        start = time.time()
        cem_agent = CEMAgent(encoder, Rssm, reward_model, args.horizon, args.N_iterations, args.N_candidates,
        ...
        for update_step in range(args.update_steps):
            ...
            embedded_observations = encoder(observations.reshape(-1, 3, 64, 64)).view(args.chunk_length,
                                                                                      args.batch_size, -1)
            ...
            for l in range(args.chunk_length - 1):
                next_state_prior, next_state_posterior, deterministic_state = Rssm(stochastic_state, actions[l],
                                                                                   deterministic_state,
                ...
                kl = kl_divergence(next_state_prior,
                                   next_state_posterior)  # 等价于 - kl(q(posterior) || p(prior)), 后面需要最小化 Loss, 符号一致
            ...
            reconstruct_observation = obs_model(flatten_stochastic_states, flatten_deterministic_states).view(
                args.chunk_length, args.batch_size, 3, 64, 64)
            predicted_reward = reward_model(flatten_stochastic_states, flatten_deterministic_states).view(
                args.chunk_length, args.batch_size, 1)
            obs_loss = 0.5 * mse_loss(reconstruct_observation[1:], observations[1:], reduction='none').mean(
                [0, 1]).sum()  # 按照 chunk 和 batch 两个维度求平均, 然后再将 3 个 channel 的所有像素的 mse loss 求和

            reward_loss = 0.5 * mse_loss(predicted_reward[1:], rewards[:-1])  # env 返回的 reward 是下一个时间步的, 手动对齐
